...
function productoToForm(p: Producto): FormData {
  return {
    ...
    codigoProducto: p.codigoProducto ?? "", codigoBarras: p.codigoBarras ?? "",
    precio: String(p.precio), costo: p.costo ? String(p.costo) : "",
    stock: String(p.stock), stockMinimo: String(p.stockMinimo),
    unidad: p.unidad ?? "", imagen: p.imagen ?? "", imagenes: p.imagenes ?? [],
...
export default function ProductoForm({ producto }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!data.ok) { setError(data.error ?? "Error al guardar el producto"); return; }
      setExito(true);
      setTimeout(() => router.push("/productos"), 1500);
    } catch (err: any) {
      if (err?.message !== "SESSION_EXPIRED") setError("Error de conexión. Intentá de nuevo.");
    } finally {
      setCargando(false);
    }
  }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {esEdicion && <p className="text-sm text-gray-500 dark:text-gray-400">{producto!.nombre}</p>}
        </div>
      </div>
        ...
        <div className="card p-5 space-y-4">
          ...
          <div>
            ...
            <input type="text" name="nombre" value={form.nombre} onChange={handleChange} required
              placeholder="Ej: Coca Cola 500ml" className="input-base" />
          </div>
          <div>
            <label className="label-base">Descripción</label>
            <textarea name="descripcion" value={form.descripcion} onChange={handleChange} rows={3}
              placeholder="Descripción opcional..." className="input-base resize-none" />
          </div>
            ...
            <div>
              ...
              {esEdicion && (
                <p className="text-xs text-gray-400 mt-1">
                  Usá <Link href="/movimientos" className="underline text-primary-600 dark:text-primary-400">Movimientos</Link> para ajustar
                </p>
              )}
            </div>
            <div>
