...
export default function ProductoAcciones({ productoId, productoNombre }: Props) {
  ...
  async function handleEliminar() {
    ...
    setError("");
    try {
      const res  = await apiFetch(`/api/productos/${productoId}`, { method: "DELETE" });
      const data = await res.json();
      if (!data.ok) { setError(data.error ?? "Error al eliminar"); return; }
      setModalAbierto(false);
      router.refresh();
    } catch (err: any) {
      // SESSION_EXPIRED ya fue manejado por apiFetch (redirect automático)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-2xl bg-white dark:bg-gray-800 shadow-xl p-6 space-y-4">
            ...
            <div className="text-center space-y-1">
              <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">¿Eliminar producto?</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Vas a desactivar{" "}
                <span className="font-semibold text-gray-700 dark:text-gray-300">{productoNombre}</span>.
                No va a aparecer más en el inventario ni en el POS.
              </p>
            </div>
            {error && (
              <p className="text-center text-xs text-red-600 bg-red-50 dark:bg-red-900/20 rounded-lg px-3 py-2">
                {error}
              </p>
            )}
            <div className="flex gap-3 pt-1">
              <button onClick={() => setModalAbierto(false)} disabled={eliminando}
