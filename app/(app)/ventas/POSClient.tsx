          ...
          {categorias.map((cat) => (
            <button key={cat.id}
              ...
              className={cn("flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-colors",
                categoriaActiva === cat.id ? "bg-red-600 text-white" : "var(--text-secondary) hover:text-white"
              )}
            ...
            {productosFiltrados.map((producto) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs font-semibold var(--text-primary) line-clamp-2 leading-tight mb-1">{producto.nombre}</p>
    ...
    <div className="flex flex-col h-full" style={{ background: "var(--bg-surface)" }}>
      ...
      <div className="hidden md:flex items-center justify-between px-4 py-3 border-b flex-shrink-0"
        ...
        <div className="flex items-center gap-2">
          <ShoppingCart className="h-5 w-5 var(--text-muted)" />
          <span className="font-semibold var(--text-primary)">Carrito</span>
          {cantidadTotal > 0 && (
            <span
        ...
        {carrito.length > 0 && (
          <button onClick={limpiarCarrito} className="text-xs var(--text-muted) hover:text-red-400 transition-colors">Limpiar</button>
        )}
      </div>
        ...
        {carrito.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full py-16 text-center px-6">
            <div className="h-16 w-16 rounded-full flex items-center justify-center mb-4" style={{ background: "var(--bg-hover)" }}>
              <ShoppingCart className="h-7 w-7 var(--text-muted)" />
            </div>
            <p className="text-sm font-medium var(--text-muted)">El carrito está vacío</p>
            ...
            {carrito.map((item) => (
              <div key={item.productoId} className="flex items-center gap-2 px-3 md:px-4 py-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium var(--text-primary) truncate">{item.nombre}</p>
                  <p className="text-xs text-zinc-500">{formatPrecio(item.precio)} c/u</p>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => cambiarCantidad(item.productoId, -1)}
                    className="flex h-6 w-6 items-center justify-center rounded-md var(--text-secondary) hover:text-white"
                    style={{ border: "1px solid var(--border-subtle)" }}>
                    <Minus className="h-3 w-3" />
                  </button>
                  <span className="w-6 text-center text-sm font-semibold var(--text-primary)">{item.cantidad}</span>
                  <button onClick={() => cambiarCantidad(item.productoId, 1)}
                    disabled={item.cantidad >= item.stock}
                    className="flex h-6 w-6 items-center justify-center rounded-md var(--text-secondary) hover:text-white disabled:opacity-30"
                    style={{ border: "1px solid var(--border-subtle)" }}>
                    <Plus className="h-3 w-3" />
                  </button>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="text-sm font-bold var(--text-primary) w-16 md:w-20 text-right">{formatPrecio(item.subtotal)}</span>
                  <button onClick={() => eliminarDelCarrito(item.productoId)} className="var(--text-muted) hover:text-red-400">
      ...
      {carrito.length > 0 && (
        <div className="border-t p-3 md:p-4 space-y-3 flex-shrink-0" style={{ borderColor: "var(--border-base)" }}>
          ...
          <div className="flex items-center gap-2">
            <label className="text-xs font-medium var(--text-muted) whitespace-nowrap">Descuento $</label>
          ...
          <div>
            <p className="text-xs font-medium var(--text-muted) mb-2">Método de pago</p>
          ...
          {metodoPago === "efectivo" && (
            <div className="flex items-center gap-2">
              <label className="text-xs font-medium var(--text-muted) whitespace-nowrap">Recibido $</label>
          ...
          <div className="space-y-1 pt-2 border-t" style={{ borderColor: "var(--border-base)" }}>
            ...
            <div className="flex justify-between font-bold text-lg var(--text-primary)">
              <span>Total</span><span className="text-red-400">{formatPrecio(total)}</span>
            </div>
            {metodoPago === "efectivo" && vuelto > 0 && (
              <div className="flex justify-between text-sm font-semibold var(--text-secondary)"><span>Vuelto</span><span>{formatPrecio(vuelto)}</span></div>
            )}
          </div>
          ...
          <label className="flex items-center gap-2.5 cursor-pointer select-none py-1" onClick={() => setImprimirTicket(v => !v)}>
            <div className="flex h-4 w-4 items-center justify-center rounded flex-shrink-0"
              style={{ background: imprimirTicket ? "#DC2626" : "transparent", border: imprimirTicket ? "1px solid #DC2626" : "1px solid var(--border-strong)" }}>
              {imprimirTicket && (
