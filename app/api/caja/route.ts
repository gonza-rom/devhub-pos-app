...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ abierta: true, caja: { ...caja, ...calcularTotales(caja.movimientos) } });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (saldoInicial < 0) return NextResponse.json({ error: "Saldo inválido" }, { status: 400 });
    const yaAbierta = await prisma.caja.findFirst({ where: { tenantId, estado: "ABIERTA" } });
    if (yaAbierta) return NextResponse.json({ error: "Ya hay una caja abierta" }, { status: 409 });
    ...
    const caja = await prisma.$transaction(async (tx) => {
      ...
      await tx.movimientoCaja.create({
        data: { tenantId, cajaId: nueva.id, tipo: "APERTURA", monto: saldoInicial,
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const cajaAbierta = await prisma.caja.findFirst({
      where: { tenantId, estado: "ABIERTA" }, include: { movimientos: true },
    });
    ...
    const caja = await prisma.$transaction(async (tx) => {
      ...
      await tx.movimientoCaja.create({
        data: { tenantId, cajaId: cajaAbierta.id, tipo: "CIERRE", monto: saldoFinal,
...
function calcularTotales(movimientos: any[]) {
